...
export async function submitFormAction(formId: string, formDataOrObj: FormData | Record<string, any>) {
            ...
            if (match && match[1]) {
                ...
                try {
                    const result = await appendToSheet({
                        ...
                        privateKey: pk,
                    }, dbData); // Use processed dbData

                    if (!result.success) {
                        return { success: false, error: 'Saved locally but failed to sync to Sheets: ' + result.error };
                    }
                } catch (e) {
                ...
                return { success: false, error: 'Invalid Google Sheet URL format. Please check the URL in form settings.' };
            }
        } else {

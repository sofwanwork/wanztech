    ...
    <div className="flex flex-col h-full bg-white border-r border-gray-200 w-64">
      ...
      <div className="flex-1 py-6 px-4 space-y-2 overflow-y-auto">
        <div className="mb-6">
          ...
          >
            <Button
              className="w-full justify-start bg-primary hover:bg-primary/90 shadow-md shadow-primary/20"
              size="lg"
            >
        ...
        <div className="space-y-1">
          ...
          {navItems.map((item) => (
            <Link key={item.href} href={item.href} className="block" onClick={onNavigate}>
              ...
              >
                <item.icon
                  className={cn('mr-3 h-5 w-5', item.active ? 'text-primary' : 'text-gray-500')}
                />
                {item.title}
              </Button>
            </Link>
      ...
      <div className="p-4 border-t border-gray-100 bg-gray-50/50">
        ...
        <form action={logoutAction}>
          <Button
            variant="ghost"
            size="sm"
            className="w-full justify-start text-gray-500 hover:text-gray-900 hover:bg-gray-200/50 h-9"
          >

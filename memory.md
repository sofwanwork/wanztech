...
## System Improvements (2026-04-02)
- **Dynamic Google Sheet Header Syncing**: Fixed a data-dropping bug where adding a new field (e.g. Email) to an existing form would cause the submission data for that field to silently disappear. The root cause was that `google-spreadsheet` only matches JSON keys to pre-existing sheet headers. Updated `lib/api/google-sheets.ts` to actively diff the incoming `data` keys against the current sheet headers, automatically appending any missing headers (e.g., `Email`) to Row 1 before inserting the new row.
- **Certificate Verification by Email (e-Sijil)**: Enhanced the public certificate check page (`app/(public)/check/[formId]/client.tsx`) and the backend action (`checkCertificateByICOrEmail` inside `actions/certificates.ts`) to support verification by either IC number or Email. The system dynamically detects the `@` symbol to switch the target Google Sheet column search context from `IC/Kad Pengenalan` to `Email/E-mel`, making the platform more versatile for international users or different form variations.
- **Pricing Synchronisation**: Synchronized the marketing descriptions in `app/pricing/page.tsx` and `components/pricing-modal.tsx` with the actual Free tier logic. 'Dynamic QR code generator', 'E-Certificate generation', and 'Certificate Verification System' were officially moved from the 'notIncluded' (X mark) block to the 'features' (Check mark) block, augmented with their respective hard limits ('5 Dynamic QR code generation', '2 E-Certificate generation') to honestly reflect capabilities to free users.
- **Pro Tier 50% Promo Activation**: Halved the Pro package promotional structure from `RM 10 then RM 19` to `RM 5 then RM 10`. The UI strike-through logic in `PlanCard` was updated to accurately reflect `RM 10` as the baseline. Display strings updated cleanly representing "50% discount for the first 3 months".
- **Automated Payment Invoicing**: Plugged the previously dormant Resend email templates (`getPaymentSuccessEmail`, `getWelcomeProEmail`) into the active backend BCL API Webhook (`app/api/payment/webhook/route.ts`). Upon receiving a successful payment payload from the gateway, the system now automatically bypasses RLS using `createAdminClient`, fetches the user's details, and dispatches a digital receipt/invoice and a "Welcome to Pro" email.

## System Improvements (2026-04-17)
- **Interactive Form Builder Onboarding**: Integrated `react-joyride` to implement an automated 5-step tutorial in the Form Builder (`app/builder/[id]/client.tsx`). The tour highlights crucial components: Form Title, Google Sheet Link, Add Question button, Drag Handle, and Save Button. It utilizes `localStorage` to ensure it only auto-plays once per browser.
- **Joyride Z-Index & Sticky Header Fix**: Resolved an overlapping issue where the Form Builder's `z-10` sticky header obscured the Joyride tooltip overlay. Instead of battling internal portal z-indexes, the sticky header was dynamically downgraded to `z-0` (`runTour ? "z-0" : "z-10"`) while the tour is active, allowing Joyride's default `z-100` portal to naturally cover it.
- **Joyride scrollOffset Configuration (V3)**: Fixed a bug where highlighted elements were getting chopped off at the top of the screen because `scrollOffset` was ignored on the `<Joyride>` root component. `react-joyride` V3 requires `scrollOffset` to be explicitly mapped onto individual items within the `steps` array. Applied a `150px` offset to center the targets comfortably below the sticky header.
- **Drag-and-Drop UX Polish**: Enhanced the visual affordance of the `@dnd-kit` drag handles in `components/forms/fields-editor/index.tsx` by adding a prominent hover state (`hover:bg-slate-200/60`, `group-hover:text-slate-700`) to make the sorting mechanism feel more interactive and discoverable.
- **Form Active/Inactive Toggle**: Introduced a new `is_active` boolean column to the `forms` table in Supabase. Added a "Status Borang (Aktif / Ditutup)" toggle in the Form Builder UI. If toggled off, the public form automatically renders a "Borang Ditutup" (Form Closed) lock screen and completely rejects backend submissions to prevent API bypass.
- **Invisible Honeypot (Anti-Bot)**: Implemented a robust spam protection mechanism by injecting an invisible `_gotcha` input field into the public form. Human users cannot see it, but automated spam bots will indiscriminately fill it. The `submitFormAction` quietly detects this field and immediately returns a fake success response, safeguarding the user's database and Google Sheets from junk data without requiring CAPTCHAs.
- **WhatsApp Viral Share Integration**: Added a "Share to WhatsApp" feature leveraging the existing `theme` JSONB column (requiring zero DB migrations). Builders can toggle `whatsappShareEnabled` and set a custom `whatsappShareMessage` in the Builder UI. When respondents successfully submit the form, a styled WhatsApp button appears on the Thank You page, allowing them to 1-click share the form link and custom message to their network via `wa.me` API.
- **Auto-Save Visual Indicator (Builder)**: Replaced the manual/invisible auto-save logic with a global debounced `useEffect` that tracks all form modifications. It dynamically renders a real-time status indicator (`Menyimpan...` -> `Tersimpan di awan`) directly in the sticky header, offering robust peace of mind to form creators.
- **Premium Respondent UX Features**: Upgraded the public form (`app/(public)/form/[id]/client.tsx`) with three major conversion-optimizing features:
  - **Dynamic Progress Bar**: A sleek progress bar now spans the top edge, recalculating in real-time as users fill out required fields, paired with a floating badge ("X / Y Terjawab") at the bottom right.
  - **Smart Auto-Scroll**: Implemented a Typeform-style auto-scroll. Whenever a user selects a single-choice answer (Radio or Rating), the viewport smoothly glides to the next question automatically, reducing manual scrolling effort.
  - **Live Countdown Timer**: If `attendanceSettings.endTime` is configured, a sticky red badge displays a real-time ticking clock ("Tutup dalam: 01h 30m 15s"). When it hits zero, it automatically invalidates the session and locks the form.

...
- **Certificate Creation Performance Fix**: Optimized the certificate creation flow which was slow due to two bottlenecks: (1) `canCreateCertificate()` / `canUpdateCertificate()` called `getCertificateTemplates()` which did a full `SELECT *` on all rows just to check `.length` — replaced with a new lightweight `getUserCertificateCount()` function that uses `SELECT COUNT(*)` (head-only query, no row data transferred). (2) Multiple redundant `getUser()` auth roundtrips in the chain (`getSubscription` → `getCertificateTemplates` → `saveCertificateTemplate` each called `getUser()` independently). The COUNT optimization alone eliminates the heaviest bottleneck (transferring full template JSON/element data across the network).
